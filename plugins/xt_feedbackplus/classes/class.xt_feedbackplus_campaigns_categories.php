...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#
# @version $Id: class.xt_feedbackplus_campaigns_categories.php 6256 2013-04-22 09:09:55Z mario $
...
class xt_feedbackplus_campaigns_categories
{
    protected $_table = TABLE_FEEDBACKPLUS_CAMPAIGNS_CATEGORIES;
    protected $_table_lang = null;
    protected $_table_seo = null;
    protected $_master_key = 'pk';
    protected $feedbackplusCampaignID = null;
    protected $nodeUrl = 'adminHandler.php?plugin=xt_feedbackplus&load_section=xt_feedbackplus_campaigns_categories&pg=getNode&';
    protected $saveUrl = 'adminHandler.php?plugin=xt_feedbackplus&load_section=xt_feedbackplus_campaigns_categories&pg=setData&';
    protected $_icons_path = 'images/icons/';

    function __construct ()
    {
        $this->indexID = time() . '-coup2cat';
        $this->nodeUrl = 'adminHandler.php?plugin=xt_feedbackplus&load_section=xt_feedbackplus_campaigns_categories&pg=getNode&';
        $this->saveUrl = 'adminHandler.php?plugin=xt_feedbackplus&load_section=xt_feedbackplus_campaigns_categories&pg=setData&';
    }
    ...
    function setFeedbackPlusCampaignId ($id)
    {
        $this->feedbackplusCampaignID = $id;
    }

    function getFeedbackPlusCampaignId ()
    {
        return $this->feedbackplusCampaignID;
    }

    function getTreePanel ()
    {
        if ($this->url_data['feedbackplus_campaign_id'])
            $this->setFeedbackPlusCampaignId($this->url_data['feedbackplus_campaign_id']);
        ...
        if ($this->getFeedbackPlusCampaignId())
            $tl->setBaseParams(array('feedbackplus_campaign_id' => $this->getFeedbackPlusCampaignId()));

        $tp = new PhpExt_Tree_TreePanel();
        $tp->setTitle(__define('TEXT_FEEDBACKPLUS_CAMPAIGNS_CATEGORIES'))
            ->setRoot($root)
            ->setLoader($tl)
            ->setAutoScroll(true)
            ->setAutoWidth(true);
        ...
        $tb->addButton(1, __define('TEXT_SAVE'), $this->_icons_path . 'disk.png', new PhpExt_Handler(PhpExt_Javascript::stm("
                 ...
                 url: '" . $this->saveUrl . "',
                 method:'POST',
                 params: {'feedbackplus_campaign_id': " . $this->getFeedbackPlusCampaignId() . ", catIds: checked},
    ...
    function getNode ()
    {
        ...
        $table_data = new adminDB_DataRead($this->_table, null, null, $this->_master_key, 'feedbackplus_campaign_id=' . $this->getFeedbackPlusCampaignId());
    ...
    function setData ($dont_die = FALSE)
    {
        ...
        if ($this->url_data['catIds'] && $this->url_data['feedbackplus_campaign_id']) {
            $db->Execute("DELETE FROM " . $this->_table . " WHERE feedbackplus_campaign_id = ?", array($this->url_data['feedbackplus_campaign_id']));
            ...
            for ($i = 0; $i < count($cat_ids); $i++) {
                ...
                if ($cat_ids[$i]) {
                    $data = array('categories_id' => (int)$cat_ids[$i], 'feedbackplus_campaign_id' => $this->url_data['feedbackplus_campaign_id']);
                    $o = new adminDB_DataSave($this->_table, $data, false, __CLASS__);
                    $obj = $o->saveDataSet();
                }
            }
