...
class N2GOApi
{
    ...
    private function getVersion(&$output){
        $output['version'] = $this->version;
    }
    
    private function testConnection(&$output) 
    {
        $output['connected'] = strlen($error) === 0;
    }
    ...
    private function getItem(&$output)
    {
        ...
        if (isset($item)) {
            ...
            $language = isset($postLanguage) ? $postLanguage : 'en';
            $i = 0;
            ...
            $sql_item = "SELECT  $select
                        ...
                        LEFT OUTER JOIN $table_manufacturers
            ...
            $images = array();
            if (count($res)) {
                if ($res[0]['xt_products.products_image'] != null) {
                    $images[] = $imgDir . $res[0]['xt_products.products_image'];
                }
                     ...
                     switch ($key) {
                        case 'xt_products.products_price':
                            if (in_array('xt_products.products_price', $attributes)) {
                                $product[$key] = (is_null($res[0][$key]) ? 0 : $res[0][$key]) * (100 + $res[0]['xt_tax_rates.tax_rate']) / 100;
                            }
                        ...
                        case 'xt_products_price_special.specials_price':
                            $price = (is_null($res[0]['xt_products.products_price']) ? 0 : $res[0]['xt_products.products_price']) * (100 + $res[0]['xt_tax_rates.tax_rate']) / 100;
                            ...
                            if (in_array('xt_products_price_special.net_price_new', $attributes)) {
                                $product['xt_products_price_special.net_price_new'] = ($special_price * 100) / ($res[0]['xt_tax_rates.tax_rate'] + 100);
                            }
                        ...
                        case 'xt_products.products_image':
                            $product[$key] = count($images) > 0 ? array_unique($images) : "";
                            break;
                        case 'xt_seo_url.url_text':
                            $product[$key] = $baseUrl . (is_null($res[0][$key]) ? "" : $res[0][$key]);
                            break;
                        default:
                            if (in_array($key, $attributes)) {
    ...
    private function getCustomers(&$output)
    {
        ...
        if ($subscribed) {
            $sql_customers .= " WHERE $table_customers.nl2go_newsletter_status='1' ";
        }
        ...
        if (!empty($groups)) {
            ...
            $sql_customers .= " $table_customers.customers_status in (" . $ids . ") ";
        }

        if ($timeframe) {
            if ($subscribed == false && empty($groups)) {
                $sql_customers .= ' WHERE ';
            } else {
                $sql_customers .= ' AND ';
            }
            ...
            $sql_customers .= " (GREATEST($table_customers.date_added, $table_customers.last_modified, $table_customers_addresses.date_added, $table_customers_addresses.last_modified ) >
    ...
    private function unsubscribeCustomer(&$output)
    {
        ...
        if (isset($postEmail)) {
            ...
            $table_customers = TABLE_CUSTOMERS;
            $sql_customers = "UPDATE $table_customers SET nl2go_newsletter_status = 0 WHERE customers_email_address = '" . $email . "'";
            $res = $this->db->Execute($sql_customers);
            $output['unsubscribe'] = $this->db->_affectedrows() ? true : false;
        }
    }

    private function getFields(&$output)
    {
        $output['fields'] = self::getCustomerFieldsArray();
    }
    ...
    private function getGroups(&$output)
    {
        ...
        $output['groups'] = $resGroups;
    }

    private function getProductAttributes(&$output)
    {
        $output['productAttributes'] = self::getProductAttributesArray();
    }
        ...
        foreach ($attributes as $attribute) {       
            ...
            switch ($attribute) {
                ...
                case 'xt_products_price_special.net_price_new':
                    if ($notInserted) {
                        $select .=  "xt_products.products_price as 'xt_products.products_price',"
                            . " xt_products_price_special.specials_price as 'xt_products_price_special.specials_price',"
                            . " xt_tax_rates.tax_rate as 'xt_tax_rates.tax_rate',";
                        $notInserted = false;
                    }
                ...
                case 'xt_seo_url.shop_url':
                    break;
                case 'xt_products.products_image':
                    $select .=  $attribute . " as '" . $attribute . "', xt_media.file as 'xt_media.file',";
                    break;
                default:
        ...
        foreach ($fields as $field) {
            ...
            switch ($field) {
                case 'xt_orders.total_order':
                    $select .= "count(xt_orders.orders_id) as '" . $field . "',";
                    break;
                case 'xt_orders_stats.total_revenue':
                    $select .= "sum(xt_orders_stats.orders_stats_price) as '" . $field . "',";
                    break;
                case 'xt_orders_stats.avg_cart':
                    $select .= "avg(xt_orders_stats.orders_stats_price) as '" . $field . "',";
                    break;
                case 'xt_orders_status_history.date_add':
                    $select .= "greatest(xt_orders_status_history.date_add) as '" . $field . "',";
                    break;
                default:
