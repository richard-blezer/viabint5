 ...
 *
 * -----------------------------------------------------------------------------
 * $Id: ComparisonShoppingCheckinSubmit.php 11 2012-01-04 12:41:15Z derpapst $
 *
...
//masoud commented		if (getDBConfigValue(array($this->settings['marketplace'].'.price.usespecialoffer', 'val'), $this->_magnasession['mpID'], false)) {
			...
			$shippingTime = MagnaDB::gi()->fetchOne(
				'SELECT shipping_status_name 
			      ...
			      WHERE shipping_status_id=\''.$product['products_shippingtime'].'\'
			      		AND language_code=\''.getDBConfigValue($this->settings['marketplace'].'.lang', $this->_magnasession['mpID']).'\'
			      LIMIT 1'
			);
		} else {
			$shippingTime = '';
