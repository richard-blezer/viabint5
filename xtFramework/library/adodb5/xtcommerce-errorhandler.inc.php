     ...
     if ($ADODB_THROW_EXCEPTIONS !==true){
		if(!function_exists('ioncube_license_properties')) {
         ...
         $lic_parms = ioncube_license_properties();
            if (!is_array($lic_parms)) {
                if (file_exists(_SRV_WEBROOT.'read_license_dev_only.php')) {
                    include(_SRV_WEBROOT.'read_license_dev_only.php');
                }
            }
        }
        ...
        if (!empty($lic_parms)){
            $s .=' \n Company: '.$lic_parms["company"]["value"];
            $s .=' \n Key: '.$lic_parms["key"]["value"];
        }
		@mail(_CORE_DEBUG_MAIL_ADDRESS,'SQL Error - xt:Commerce VEYTON',$s);
	}
