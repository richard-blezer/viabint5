 ...
 * @example   use in templates like {hook key=ew_viabiona_seotext}
 *
...
if (class_exists('ew_viabiona\plugin') && ew_viabiona_plugin::status()) {
    ...
    if (ew_viabiona_plugin::show_seotext()) {
        $tpl_object = new template();
        $tpl = 'ew_viabiona_seotext.html';
        $tpl_data = array();
        ...
        if (!ew_viabiona_plugin::isFileCacheAllowed() || !$tpl_object->isTemplateCache($tpl)) {
            ...
            $dataFormatted = null;
            if ($data = $ew_viabiona_plugin->getContentsByBlock('ew_viabiona_seotext', false)) {
            ...
            $output = $tpl_object->getTemplate('ew_viabiona_seotext', $tpl, $tpl_data);
        } else {
