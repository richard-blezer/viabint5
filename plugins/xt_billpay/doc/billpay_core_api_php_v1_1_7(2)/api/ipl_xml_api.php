...
define('IPL_CORE_API_VERSION', 							"1.1.7");
...
define('IPL_CORE_PAYMENT_TYPE_PARTIAL_PAYMENT',			3);
...
function ipl_core_send_curl_request($requestUrl, $requestData) {
	...
	if (!$result) {
		$ipl_core_error_code = 1;
		$ipl_core_error_msg = 'Connection timeout while connecting billpay server';
		return false;
	}
...
function ipl_core_send_module_config_request($requestUrlBase, $defaultParams) {
	$defaultParamsXml = ipl_core_build_closed_tag("default_params", $defaultParams);
	return ipl_core_generic_send_request($requestUrlBase, 'moduleConfig', array(), array($defaultParamsXml), 'ipl_core_parse_module_config_response');
}

function ipl_core_send_preauthorize_request($requestUrlBase, $attributes, $defaultParams, $customerDetails, 
	$shippingDetails, $bankAccount, $totals, $articleData, $orderHistoryData) {
	$defaultParamsXml 			= ipl_core_build_closed_tag("default_params", $defaultParams);
	...
	return ipl_core_generic_send_request($requestUrlBase, 'preauthorize', $attributes, array($defaultParamsXml, $customerDetailsXml, 
			$shippingDetailsXml, $bankAccountXml, $totalsXml, $articleDataXml, $historyDataXml), 'ipl_core_parse_preauthorize_response');
}
...
function ipl_core_send_capture_request($requestUrlBase, $defaultParams, $captureParams) {
	$defaultParamsXml = ipl_core_build_closed_tag("default_params", $defaultParams);
	$captureParamsXml = ipl_core_build_closed_tag("capture_params", $captureParams);
	return ipl_core_generic_send_request($requestUrlBase, 'capture', array(), array($defaultParamsXml, $captureParamsXml), 'ipl_core_parse_capture_response');
}

function ipl_core_send_invoice_request($requestUrlBase, $defaultParams, $invoiceParams) {
	$defaultParamsXml = ipl_core_build_closed_tag("default_params", $defaultParams);
	$invoiceParamsXml = ipl_core_build_closed_tag("invoice_params", $invoiceParams);
	return ipl_core_generic_send_request($requestUrlBase, 'invoiceCreated', array(), array($defaultParamsXml, $invoiceParamsXml), 'ipl_core_parse_invoice_response');
}

function ipl_core_send_update_order_request($requestUrlBase, $defaultParams, $updateParams) {
	$defaultParamsXml = ipl_core_build_closed_tag("default_params", $defaultParams);
	$udpateParamsXml = ipl_core_build_closed_tag("update_params", $updateParams);
	return ipl_core_generic_send_request($requestUrlBase, 'updateOrder', array(), array($defaultParamsXml, $udpateParamsXml), 'ipl_core_parse_upate_order_response');
}
