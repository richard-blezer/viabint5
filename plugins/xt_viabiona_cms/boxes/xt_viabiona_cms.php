...
if(!isset($xtPlugin->active_modules['xt_viabiona_cms'])) 
...
else 
    {
		...
		$cat_main = $category->getCategoryBox(240);
		$cat_main1 = $category->getCategoryBox(230);
		$cat_main2 = $category->getCategoryBox(124);
		$cat_main3 = $category->getCategoryBox(122);
		...
		foreach ($cat_main1 as $key1=>$cat1) 
		{
			$is_active = 0;
			foreach ($cat_main1 as $key2=>$cat2) {
				if ($cat2['level'] == '2' && $cat2['active'] == '1') {
					if ($cat1['categories_id'] == $cat2['parent_id']) {
						$is_active = 1;
					}
			...
			$cat_main1[$key1]['is_sub_active'] = $is_active;
		}
		
		foreach ($cat_main2 as $key1=>$cat1) 
		{
			$is_active = 0;
			foreach ($cat_main2 as $key2=>$cat2) {
				if ($cat2['level'] == '2' && $cat2['active'] == '1') {
					if ($cat1['categories_id'] == $cat2['parent_id']) {
						$is_active = 1;
					}
			...
			$cat_main2[$key1]['is_sub_active'] = $is_active;
		}
		
		foreach ($cat_main3 as $key1=>$cat1) 
		{
			$is_active = 0;
			foreach ($cat_main3 as $key2=>$cat2) {
				if ($cat2['level'] == '2' && $cat2['active'] == '1') {
					if ($cat1['categories_id'] == $cat2['parent_id']) {
						$is_active = 1;
					}
			...
			$cat_main3[$key1]['is_sub_active'] = $is_active;
		}

		$cat1 = $category->getCategoryBox(124); 
		...
		$tpl_data['cat_main1'] = $cat_main1;
		$tpl_data['cat_main2'] = $cat_main2;
		$tpl_data['cat_main3'] = $cat_main3;
