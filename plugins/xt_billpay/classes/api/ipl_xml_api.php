...
define('IPL_CORE_API_VERSION', 							"1.3.6");
...
function ipl_core_send_preauthorize_request($requestUrlBase, $attributes, $defaultParams, $customerDetails, 
	$shippingDetails, $bankAccount, $totals, $articleData, $orderHistoryData, $rateRequestData, $companyDetails) {
	$defaultParamsXml 			= ipl_core_build_closed_tag("default_params", $defaultParams);
...
function ipl_core_send_calculate_rates_request($requestUrlBase, $defaultParams, $rateParams) {
	$defaultParamsXml = ipl_core_build_closed_tag("default_params", $defaultParams);
	$rateParamsXml = ipl_core_build_closed_tag("rate_params", $rateParams);
	return ipl_core_generic_send_request($requestUrlBase, 'calculateRates', array(), array($defaultParamsXml, $rateParamsXml), 'ipl_core_parse_calculate_rates_response');
}
