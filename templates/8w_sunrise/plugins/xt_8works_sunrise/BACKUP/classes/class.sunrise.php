...
class sunrise {
	...
	function recurse_array_to_html_list($params) {
		...
		if (is_array($params['array']) && $count > 0) {
			...
			$output .= '<div class="catcount-'.$count.$ul_class.'">';
			...
			foreach ($params['array'] as $element) {
				...
				//item html
				$output .= '<div id="cat-id-'.$element['categories_id'].'" class="category level-'.$element['level'].' ul-item-'.$i.' lang-'.$element['language_code'].$act.$pos.$sub.'">';
