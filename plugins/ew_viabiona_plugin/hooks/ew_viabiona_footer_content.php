...
/**
 * Footer Content Block `ew_viabiona_footer_content`
 *
 * @author    Jens Albert
 * @copyright 8works <info@8works.de>
 * @example   use in templates like {hook key=ew_viabiona_footer_content}
 *
...
if (class_exists('ew_viabiona\plugin') && ew_viabiona_plugin::status()) {
    ...
    $tpl = 'ew_viabiona_footer_content.html';
    ...
    if (!ew_viabiona_plugin::isFileCacheAllowed() || !$tpl_object->isTemplateCache($tpl)) {
        ...
        if ($data = $ew_viabiona_plugin->getContentsByBlock('ew_viabiona_footer_content', false)) {
            $output = $tpl_object->getTemplate('ew_viabiona_footer_content', $tpl, array('data' => $data));
            unset($data);
        }
