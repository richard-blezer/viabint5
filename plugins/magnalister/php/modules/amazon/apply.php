...
function populateGenericData($pID, $edit = false) {
	...
	$product = MLProduct::gi()->getProductByIdOld(
		$pID, getDBConfigValue('amazon.lang', $_MagnaSession['mpID'], $_SESSION['magna']['selected_language'])
	);
	if ($product === false) {
		return $genericDataStructure;
	}
	...
	$product['meta_keywords'] = explode(',', $product['meta_keywords']);
	array_walk($product['meta_keywords'], $trimFunc);
	$genericDataStructure['Keywords'] = array_slice($product['meta_keywords'], 0, 5);
	...
	if ($dbData !== false) {
		...
		$genericDataStructure['LeadtimeToShip'] = $genericDataStructure['leadtimeToShip'];
