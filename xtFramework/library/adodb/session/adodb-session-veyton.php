...
*/
    ...
    */
    static function read($key) 
    {
        ...
        $sql = "SELECT sessdata FROM $table WHERE sesskey = $qkey AND expiry >= " . $conn->sysTimeStamp;
    ...
    */
    ...
    global $ADODB_SESSION_READONLY;
        ...
        if ($crc !== false && $crc == (strlen($oval) . crc32($oval))) {
            ...
            $sql = "UPDATE $table SET expiry = $expiry ,expireref=".$conn->Param('0').", modified = $sysTimeStamp WHERE sesskey = ".$conn->Param('1')." AND expiry >= $sysTimeStamp";
        ...
        if (!$clob) {    // no lobs, simply use replace()
            $rs = $conn->Execute("SELECT COUNT(*) AS cnt FROM $table WHERE sesskey = ".$conn->Param(0),array($key));
            if ($rs) $rs->Close();
        ...
        if (!$rs) {
            ...
            if ($conn->databaseType == 'access') {
                $sql = "SELECT sesskey FROM $table WHERE sesskey = $qkey";
                $rs = $conn->Execute($sql);
                ADODB_Session::_dumprs($rs);
                if ($rs) {
                    $rs->Close();
    ...
    */
    static function destroy($key) {
        ...
        if ($expire_notify) {
            ...
            $sql = "SELECT expireref, sesskey FROM $table WHERE sesskey = $qkey";
            $rs = $conn->Execute($sql);
            ADODB_Session::_dumprs($rs);
        ...
        $sql = "DELETE FROM $table WHERE sesskey = $qkey";
        $rs = $conn->Execute($sql);
        ADODB_Session::_dumprs($rs);
        if ($rs) {
            $rs->Close();
