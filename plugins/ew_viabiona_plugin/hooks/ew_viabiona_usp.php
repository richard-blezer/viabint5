 ...
 * @example   use in templates like {hook key=ew_viabiona_usp}
 *
...
if (class_exists('ew_viabiona\plugin') && ew_viabiona_plugin::status()) {
    ...
    if (ew_viabiona_plugin::show_usp()) {
        $position = (isset($params['position']) && !empty($params['position']) && $params['position'] != 'default') ? '_' . trim($params['position']) : '';
        $tpl_object = new template();
        $tpl = "ew_viabiona_usp{$position}.html";
        $tpl_data = array();
        ...
        if (!ew_viabiona_plugin::isFileCacheAllowed() || !$tpl_object->isTemplateCache($tpl)) {
            ...
            $dataFormatted = null;
            if ($data = $ew_viabiona_plugin->getContentsByBlock("ew_viabiona_usp{$position}", false)) {
