	...
	protected function renderSinglePrepareView($data, $preSelected) {
		...
		<tbody>
			...
			<tr class="<?= ($oddEven = !$oddEven) ? 'odd' : 'even' ?> langde">
				...
				<td class="input">
					<input type="text" class="fullwidth" name="TitleDe" id="TitleDe" value="<?= fixHTMLUTF8Entities($data['TitleDe'], ENT_COMPAT) ?>"/>
				</td>
				<td class="info"></td>
			</tr>
			<tr class="<?= ($oddEven = !$oddEven) ? 'odd' : 'even' ?> langde">
				...
				<td class="input">
					<input type="text" class="fullwidth" name="SubtitleDe" id="SubtitleDe" value="<?= isset($data['SubtitleDe']) ? fixHTMLUTF8Entities($data['SubtitleDe'], ENT_COMPAT) : '' ?>"/>
				</td>
				<td class="info"></td>
			</tr>
			<tr class="<?= ($oddEven = !$oddEven) ? 'odd' : 'even' ?> langde">
			...
			<tr class="<?= ($oddEven = !$oddEven) ? 'odd' : 'even' ?> langfr">
				...
				<td class="input">
					<input type="text" class="fullwidth" name="TitleFr" id="TitleFr" value="<?= fixHTMLUTF8Entities($data['TitleFr'], ENT_COMPAT) ?>"/>
				</td>
				<td class="info"></td>
			</tr>
			<tr class="<?= ($oddEven = !$oddEven) ? 'odd' : 'even' ?> langfr">
				...
				<td class="input">
					<input type="text" class="fullwidth" name="SubtitleFr" id="SubtitleFr" value="<?= isset($data['SubtitleFr']) ? fixHTMLUTF8Entities($data['SubtitleFr'], ENT_COMPAT) : '' ?>"/>
				</td>
				<td class="info"></td>
			</tr>
			<tr class="<?= ($oddEven = !$oddEven) ? 'odd' : 'even' ?> langfr">
	...
	protected function renderMultiPrepareView($data, $preSelected) {
		...
		if (count($data) === 1) {
			...
			$ricardoPrice = $this->price
				->setFinalPriceFromDB($aProduct['ProductId'], $this->mpID)
				->getPrice();
				...
				<tr class="' . (($oddEven = !$oddEven) ? 'odd' : 'even') . '">
					...
					<td class="input">';
						$durationSelect = '<div style="float: left;">'
								. '<select id="config_' . $this->marketplace . '_duration" name="conf[' . $this->marketplace . '.duration]">';
		...
		$html .= $secondPromotionSelect . '
			...
			<tbody>
				<tr class="headline">
					<td colspan="3"><h4>'.'Weitere Einstellungen'.'</h4></td>
				</tr>
				<tr class="' . (($oddEven = !$oddEven) ? 'odd' : 'even') . '">
			...
			$(document).ready(function() {
				...
				$('#config_<?= $this->marketplace ?>_checkin_buyingmode').change(function() {
					if (this.value === 'auction') {
						...
						<?php else : ?>
						$('#config_<?= $this->marketplace ?>_enable_price_buy_now_price').closest('.odd').show();
						<?php endif ?>
					} else {
