 ...
 *
 * -----------------------------------------------------------------------------
 * $Id: PrepareCategoryView.php 197 2013-03-07 17:03:36Z tim.neumann $
 *
...
class PrepareCategoryView extends QuickCategoryView {
	...
	public function getAdditionalProductInfo($pID, $data = false) {
		...
		if (empty($a)) {
			return '
				<td>&mdash;</td>
				<td>'.html_image(DIR_MAGNALISTER_IMAGES . 'status/grey_dot.png', ML_EBAY_PRODUCT_MATCHED_NO, 9, 9).'</td>';
		}
		...
		if ('OK' != $a['Verified']) {
			return '
				...
				<td>'.html_image(DIR_MAGNALISTER_IMAGES . 'status/red_dot.png', ML_EBAY_PRODUCT_PREPARED_FAULTY, 9, 9).'</td>';
		}
		return '
			<td '.$priceTooltip.'>'.$startPriceFormat.$textEBayPrice.$endPriceFormat.'</td>
			<td>'.html_image(DIR_MAGNALISTER_IMAGES . 'status/green_dot.png', ML_EBAY_PRODUCT_PREPARED_OK, 9, 9).'</td>';
	}
