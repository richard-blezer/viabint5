...
// only show button if there is a authorization id
    ...
    $rs = $db->GetRow($query);
        ...
        if (count($rs)>0 && $rs['XT_PAYMENTS_AUTHORIZATION_AMOUNT']>0) {

$js .= "Ext.Msg.show({
		   title:'".TEXT_START."',
		   msg: '".XT_PAYMENTS_TEXT_PAYPAL_GET_FUNDS_ASK."',
...
$js .= "function doCapture(order_id, btn){
			  		if (btn == 'yes') {
		                 ...
		                 conn.request({
		                 url: '../plugins/xt_payments/scripts/doCapture.php',
		                 method:'GET',
...
$paypalCaptureButton = PhpExt_Button::createTextButton(__define("XT_PAYMENTS_BUTTON_CAPTURE_PAYPAL_FUNDS"),
