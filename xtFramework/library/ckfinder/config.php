...
$xtclass = (strpos($_GET['type'], 'files') === 0)
	? 'MediaFiles'
	: 'MediaImages';
//var_dump('<pre>', $_GET); exit;
$xtclass = new $xtclass();
$xtclass->setClass($_GET['type']);
$xtclass->setUrlData($_GET);
$allowedExtensions = str_replace('|', ',', $xtclass->FileTypes);
...
$config['LicenseKey'] = 'Q2QX-QXW4-M4P2-GTSU-U336-G31G-UFDS';
...
$config['Thumbnails'] = Array(
		'url' => $baseUrl.'images/_ckthumb',
		'directory' => $baseDir.'images/_ckthumb',
		'enabled' => true,
		'directAccess' => false,
		'maxWidth' => 100,
		'maxHeight' => 100,
...
$config['ResourceType'][] = array(
	'name' => $_GET['type'],
	'url' => $baseUrl.(strpos($_GET['type'], 'files') === 0 ? 'files' : 'images/org'),
	'directory' => $baseDir.(strpos($_GET['type'], 'files') === 0 ? 'files' : 'images/org'),
	'maxSize' => 0,
	'allowedExtensions' => str_replace('|', ',', $xtclass->FileTypes),
	'deniedExtensions' => ''
);
//var_dump($config['ResourceType']); exit;
/*
global $db;
$rs = $db->Execute("SELECT GROUP_CONCAT(file_ext) AS exts FROM ".TABLE_MEDIA_FILE_TYPES." WHERE file_type IN('files', 'images') GROUP BY file_type ORDER BY file_type ASC");
if ($rs->RecordCount() != 2)
{
	$filesAllowedExtensions = 'pdf,3gp,ogg,mp4,dmg,mp3,rar,xls,gz,exe,jpeg,doc,zip,aac';
	$imagesAllowedExtensions = 'jpg,png,gif';
}
else
{
	$filesAllowedExtensions = $rs->fields['exts'];
	$rs->MoveNext();
	$imagesAllowedExtensions = $rs->fields['exts'];
}

$config['ResourceType'][] = Array(
		'name' => 'Files_order',				// Single quotes not allowed
		'url' => $baseUrl . 'files',
		'directory' => $baseDir . 'files',
		'maxSize' => 0,
		'allowedExtensions' => $filesAllowedExtensions,
		'deniedExtensions' => ''
);

$config['ResourceType'][] = Array(
		'name' => 'Files_free',				// Single quotes not allowed
		'url' => $baseUrl . 'files_public',
		'directory' => $baseDir . 'files_public',
		'maxSize' => 0,
		'allowedExtensions' => $filesAllowedExtensions,
		'deniedExtensions' => ''
);
...
$config['ResourceType'][] = Array(
		'name' => 'Default',
		'url' => $baseUrl.'images/org/',
		'directory' => $baseDir.'images/org/',
		'maxSize' => 0,
		'allowedExtensions' => $imagesAllowedExtensions,
		'deniedExtensions' => ''
);

$config['ResourceType'][] = Array(
		'name' => 'Product',
		'url' => $baseUrl.'images/product/',
		'directory' => $baseDir.'images/product/',
		'maxSize' => 0,
		'allowedExtensions' => $imagesAllowedExtensions,
		'deniedExtensions' => ''
);
...
$config['ResourceType'][] = Array(
		'name' => 'Category',
		'url' => $baseUrl.'images/category/',
		'directory' => $baseDir.'images/category/',
		'maxSize' => 0,
		'allowedExtensions' => $imagesAllowedExtensions,
		'deniedExtensions' => ''
);
...
$config['ResourceType'][] = Array(
		'name' => 'Manufacturer',
		'url' => $baseUrl.'images/manufacturer/',
		'directory' => $baseDir.'images/manufacturer/',
		'maxSize' => 0,
		'allowedExtensions' => $imagesAllowedExtensions,
		'deniedExtensions' => ''
);
...
$config['ResourceType'][] = Array(
		'name' => 'Content',
		'url' => $baseUrl.'images/content/',
		'directory' => $baseDir.'images/content/',
		'maxSize' => 0,
		'allowedExtensions' => $imagesAllowedExtensions,
		'deniedExtensions' => ''
);
...
*/
$config['HideFolders'] = Array(".*", "CVS", "icon", "info", "thumb", "popup", "_ckthumb");
...
{
	include_once "plugins/xtCommerceInsertDB/plugin.php";
}
