...
/**
 * Footer Content Block `ew_viabiona_footer_mobile_infocenter`
 *
 * @author    Jens Albert
 * @copyright 8works <info@8works.de>
 * @example   use in templates like {hook key=ew_viabiona_footer_mobile_infocenter}
 *
...
if (class_exists('ew_viabiona\plugin') && ew_viabiona_plugin::status()) {
    ...
    $tpl = 'ew_viabiona_footer_mobile_infocenter.html';
    ...
    if (!ew_viabiona_plugin::isFileCacheAllowed() || !$tpl_object->isTemplateCache($tpl)) {
        ...
        if ($data = $ew_viabiona_plugin->getContentsByBlock('ew_viabiona_footer_mobile_infocenter', false)) {
            $output = $tpl_object->getTemplate('ew_viabiona_footer_mobile_infocenter', $tpl, array('data' => $data));
            unset($data);
        }
