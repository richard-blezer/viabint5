			...
			if ($itemsApplied['itemsCount'] == 0) {
				...
				return $html.'
					<td title="'.ML_MEINPAKET_LABEL_CATMATCH_NOT_PREPARED.'">'.
						html_image(DIR_MAGNALISTER_IMAGES . 'status/grey_dot.png', ML_MEINPAKET_LABEL_CATMATCH_NOT_PREPARED, 9, 9).
					'</td>';
			}
			if ($itemsApplied['incompleteCount'] == $totalItems) {
				...
				return $html.'
					<td title="'.ML_MEINPAKET_LABEL_CATMATCH_PREPARE_INCOMPLETE.'">'.
						html_image(DIR_MAGNALISTER_IMAGES . 'status/red_dot.png', ML_MEINPAKET_LABEL_CATMATCH_PREPARE_INCOMPLETE, 9, 9).
					'</td>';
			}
			if (($itemsApplied['itemsCount'] == $totalItems) && ($itemsApplied['incompleteCount'] == 0)) {
				...
				return $html.'
					<td title="'.ML_MEINPAKET_LABEL_CATMATCH_PREPARE_COMPLETE.'">'.
						html_image(DIR_MAGNALISTER_IMAGES . 'status/green_dot.png', ML_MEINPAKET_LABEL_CATMATCH_PREPARE_COMPLETE, 9, 9).
					'</td>';
			}
			if ($itemsApplied['itemsCount'] > 0) {
				...
				return $html.'
					<td title="'.ML_MEINPAKET_LABEL_CATMATCH_PREPARE_INCOMPLETE.'">'.
						html_image(DIR_MAGNALISTER_IMAGES . 'status/yellow_dot.png', ML_MEINPAKET_LABEL_CATMATCH_PREPARE_INCOMPLETE, 9, 9).
					'</td>';
			}
		}
		return $html.'
		...
		if ($a !== false) {
			if ($a['mp_category_id'] != '') {
				return '
					...
					<td>'.html_image(DIR_MAGNALISTER_IMAGES . 'status/green_dot.png', ML_MEINPAKET_LABEL_CATMATCH_PREPARE_COMPLETE, 9, 9).'</td>';				
			} else {
				return '
					<td>'.$this->renderCatBlock($a).'</td>
					<td>'.html_image(DIR_MAGNALISTER_IMAGES . 'status/red_dot.png', ML_MEINPAKET_LABEL_CATMATCH_PREPARE_INCOMPLETE, 9, 9).'</td>';				
			}
		}
		return '
			<td>&mdash;</td>
			<td>'.html_image(DIR_MAGNALISTER_IMAGES . 'status/grey_dot.png', ML_MEINPAKET_LABEL_CATMATCH_NOT_PREPARED, 9, 9).'</td>';
	}
