   ...
   */
	...
	* then we can scale to 12.8 million unique cached recordsets. Wow!
	...
	function CacheExecute($secs2cache,$sql=false,$inputarr=false)
	{
		...
		// xt:Commerce GmbH	, performance Update 4.0.10
		if (_ACTIVATE_DB_CACHE==0) {
	...
	function AutoExecute($table, $fields_values, $mode = 'INSERT', $where = FALSE, $forceUpdate=true, $magicq=false) 
	{
		...
		function ExecuteInsertOrUpdate($table, $fields_values)
		{

			if ($this->databaseType != 'mysql') {
				die ("$this->databaseType is not supported by this function!");
			}
			foreach ($fields_values as $k => $v) {
				$cols[] = $k;
				if (! is_numeric ($v)) {
					if (is_null ($v)) {
						$fields_values[$k] = 'null';
					}
					else {
						$fields_values[$k] = $this->qstr ($v);
					}
				}
				$vals[] = $fields_values[$k];
				$ups[] = "$k=" . $fields_values[$k];
			}
			
			$sql = 'insert into ' . $table . '(' . implode (',', $cols) .
				') values (' . implode (',', $vals) . ') ' .
				'ON DUPLICATE KEY UPDATE ' . implode (',', $ups);
