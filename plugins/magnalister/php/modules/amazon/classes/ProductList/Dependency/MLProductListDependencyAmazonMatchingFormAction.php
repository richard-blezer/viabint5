		...
		if (!empty($pIDs)) {
			...
			if ( MLProduct::gi()->hasMasterItems()) {
				...
				", false), true) as $iMaster) {
					$aMaster = MLProduct::gi()->setLanguage(getDBConfigValue('amazon.lang', $this->getMagnaSession('mpID'), $_SESSION['magna']['selected_language']))->getProductById($iMaster);
					$aVariants = array();
					...
					if(
						empty($aVariants) 
						|| 0 == MagnaDb::gi()->fetchOne("SELECT COUNT(".$sKeyType.") FROM ".TABLE_MAGNA_AMAZON_PROPERTIES." WHERE mpID='".$this->getMagnaSession('mpID')."' AND ".$sKeyType." in('".implode("', '", $aVariants)."')")
					) {
